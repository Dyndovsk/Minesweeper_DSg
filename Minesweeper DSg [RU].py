...
def opt_user():
    ...
    if user == 'A':
        ...
        messagebox.showerror('ОШИБКА!', 'БУКВА ВВЕДЕНА НЕ ВЕРНО! ПЕРЕЗАПУСТИТЕ ПРОГРАММУ! (используйте Caps Look)')
...
def press(X):
    ...
    if globals()[X] == 1:
        b_X.config(text='💥')
        messagebox.showerror("Внимание","Вы проиграли!")
        bomb = 1
        sys.exit()
    elif globals()[X] == 0:
        b_X.config(text='🕳')
...
def robot():
    ...
    if rob == 1:
        if A == 1:
            b_A.config(text='💥')
            messagebox.showerror("Внимание", "Бот проиграл!")
            bomb = 1
            sys.exit()
        elif oneone == 0:
            robot()
...
button_manual = tk.Button(root, text = 'Правила игры и инструкция', command= manual)    
heading_manual = tk.Label(root, text = 'Правила игры:')
manual_text = tk.Text(root, wrap=tk.NONE, height=5, width=75)
manual_text.insert(tk.END, """В начале игры программа спрашивает у вас клетку, куда вы поставите бомбу.
Писать на английской раскладке, заглавными (большими) буквами.
Бот тоже выбирает клетку для бомбы
Потом вы по очереди выбираете клетку, чтобы проверить, есть ли там бомба
Приятной игры!""")
manual_text.config(state=tk.DISABLED)
button_main = tk.Button(root, text = 'На главную', command= start_window)
...
label_bomb = tk.Label(root, text="Введите место для бомбы:")
entry_bomb = tk.Entry(root)
laying_label = tk.Label(root, text = 'ABCD\nEFGH\nI J K L\nMNOP')
laying_button = tk.Button(root, text='Выбрать', command=opt_user)
